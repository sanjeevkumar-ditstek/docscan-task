...
import { createSchema, getSchema, loginSchema, updateSchema } from "../../utils/common/joiSchema/user/userSchema";
...
export default class UserService implements IUserService.IUserServiceAPI {
    ...
    public loginUser = async (req: IUserService.ILoginUserRequest, res: IUserService.ILoginUserResponse) => {
        ...
        try {
            ...
            return apiResponse(response)
                ...
                if(result.valid) {
                    let user: IUSER = await this.userStore.getByEmail(result?.payload?.email);
                    if(user) {
                        const payload = {
                       ...
                       return apiResponse(response)
                        ...
                        const userBySocialToken: IUSER = await this.userStore.getBySocialToken(payload)
                        if(userBySocialToken) {
                           ...
                           const user: IUSER = await this.userStore.update(userBySocialToken._id, payload) 
                           let token: string = this.generateJWT(user);
                           response.statusCode = STATUS_CODES.OK;
                           response.message = responseMessage.USER_LOGIN
                           response.data = { user, token }
    ...
    * Update
    */
    public update = async (req: IUserService.IUpdateUserRequest,
        ...
        try {
            ...
            let updateUser: IUSER = await this.userStore.update(id, value);
            response.statusCode = STATUS_CODES.OK
    ...
    * Delete
    */
    public delete = async (request: IUserService.IDeleteUserRequest, res: IUserService.IDeleteUserResponse) => {
        const response: IApiResponse = {
            response: res,
            statusCode: STATUS_CODES.INTERNAL_SERVER_ERROR,
            message: ErrorMessageEnum.INTERNAL_ERROR,
