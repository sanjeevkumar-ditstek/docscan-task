...
export default class UserStore {
	...
	public async getByEmail(email: string): Promise<IUSER> {
		try {
			let user: any = await UserModel.findOne({ email, status: { $ne: Status.DELETED } },{password: 1, firstname: 1, lastname: 1, email: 1});
			return user;
		} catch (e) {
	...
	public async getById(id: string): Promise<IUSER> {
		try {
			let user: any = await UserModel.findOne({ _id: id, status: { $ne: Status.DELETED } });
			return user;
		} catch (e) {
			return Promise.reject(new UserStore.OPERATION_UNSUCCESSFUL());
		}
	...
	public async getAll(): Promise<IUSER[]> {
		try {
			let users: any = await UserModel.find({status: { $ne: Status.DELETED }});
			return users
		} catch (e) {
			return Promise.reject(new UserStore.OPERATION_UNSUCCESSFUL());
		}
	...
	public async update(id: string, payload: IUSER): Promise<IUSER> {
		try {
			let user: IUSER = await UserModel.findOneAndUpdate({ _id: id }, payload, { new: true });
			return user
		} catch (e) {
			return e;
		}
	...
	public async delete(id: string): Promise<IUSER> {
		try {
			let user: any = await UserModel.findOneAndUpdate({ _id: id }, { status: Status.DELETED }, { new: true });
			return user
		} catch (e) {
			return e
		}
